...
def editarPersona(request,id):
    ...
    if request.method == 'POST':
        formaPersona = PersonaForm(request.POST, instance=personas)
        if formaPersona.is_valid():
            formaPersona.save()
            return redirect('inicio')

    else:
        formaPersona = PersonaForm(instance=personas)
...
def editarDomicilio(request,id):
    ...
    return render(request, 'personas/editar_domicilios.html', {'formaDomicilio': formaDomicilio})
...
def agregarDomicilio(request):
    ...
    return render(request, 'personas/agregar_domicilio.html',{'formaDomicilio':formaDomicilio})
...
def eliminarDomicilio(request,id):
    ...
    return render(request,'personas/editar_domicilio',{'formaPersona': formaPersona})
